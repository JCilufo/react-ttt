...
describe("calculateWinner", () => {
  ...
  describe("O wins", () => {
    ...
    test("middle row", () => {
      const board = ["X", "X", null, "O", "O", "O", null, null, null];
      expect(calculateWinner(board)).toEqual({ winner: "O", line: [3, 4, 5] });
    });

    test("bottom row", () => {
      const board = ["X", "X", null, null, null, null, "O", "O", "O"];
      expect(calculateWinner(board)).toEqual({ winner: "O", line: [6, 7, 8] });
    });
    ...
    test("middle column", () => {
      ...
      expect(calculateWinner(board)).toEqual({ winner: "O", line: [1, 4, 7] });
    });

    test("right column", () => {
      const board = ["X", "X", "O", null, null, "O", null, null, "O"];
      expect(calculateWinner(board)).toEqual({ winner: "O", line: [2, 5, 8] });
    });
